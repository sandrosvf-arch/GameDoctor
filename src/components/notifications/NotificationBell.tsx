...
export function NotificationBell() {
  ...
  return (
    <div className="relative" style={{ zIndex: 3 }}>
      ...
      {open ? createPortal(
        <div className="fixed bottom-4 left-2 right-2 top-[4.5rem] z-[1000] flex max-h-[calc(100dvh-5.5rem)] min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl md:bottom-auto md:left-auto md:right-4 md:top-16 md:max-h-none md:w-[min(24rem,calc(100vw-2rem))]">
          <div className="flex min-w-0 items-center justify-between gap-2 border-b border-border px-4 py-3">
            ...
            <button type="button" disabled={!unreadCount || marking} onClick={() => void markRead()} className="flex shrink-0 items-center gap-1 whitespace-nowrap text-[11px] text-cyan-300 disabled:opacity-40"><CheckCheck className="h-3.5 w-3.5" /> Marcar todas</button>
          </div>
