...
export async function POST(request: Request) {
  ...
  if (file.size > MAX_UPLOAD_BYTES) {
    return NextResponse.json({ error: "A imagem deve ter no m?ximo 5 MB." }, { status: 400 })
  }
  ...
  const filePath = "avatars/" + session.user.id + "/" + Date.now() + "-" + randomUUID() + "." + extension
  ...
  if (error) {
    console.error("[member/profile/avatar] Supabase upload failed", error)
