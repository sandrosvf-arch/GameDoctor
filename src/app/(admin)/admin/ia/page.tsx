...
type Conversation = {
  ...
  createdAt: string
  updatedAt: string
  user: { name: string | null; email: string; subscriptionActive: boolean }
...
function SubscriptionBadge({ active }: { active: boolean }) {
  return (
    <span className={`inline-flex shrink-0 rounded-full border px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.08em] ${active ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-slate-400/20 bg-slate-400/[0.08] text-slate-400"}`}>
      {active ? "Assinante" : "Sem plano"}
    </span>
  )
}
...
export default function AdminAiHistoryPage() {
  ...
  async function load(targetPage = 1) {
    setLoading(true)
    const response = await fetch(`/api/admin/ia/conversas?q=${encodeURIComponent(query)}&page=${targetPage}&pageSize=15`, { cache: "no-store" })
    const data = await response.json().catch(() => null)
    const conversations = response.ok ? data?.conversations ?? [] : []
    setItems(conversations)
    setPagination(response.ok ? data?.pagination ?? null : null)
    ...
    <main className="flex min-h-full flex-col bg-background p-4 md:p-8">
      ...
      <section className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 overflow-hidden rounded-2xl border border-border bg-card shadow-2xl shadow-black/10">
        <aside className={`${selected ? "hidden md:flex" : "flex"} w-full shrink-0 flex-col border-r border-border md:w-[350px] lg:w-[390px]`}>
           <div className="border-b border-border p-4"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void load() }} placeholder="Buscar conversa ou usuário" className="h-10 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-sm outline-none transition focus:border-cyan-400/50" /></div><div className="mt-3 flex items-center justify-between text-xs text-muted-foreground"><span>{pagination?.totalItems ?? items.length} conversa{(pagination?.totalItems ?? items.length) === 1 ? "" : "s"}</span><button type="button" onClick={() => { setQuery(""); void load() }} className="inline-flex items-center gap-1 hover:text-foreground"><X className="h-3.5 w-3.5" /> Limpar</button></div></div>
           <div className="min-h-0 flex-1 overflow-y-auto">{loading ? <div className="flex h-40 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div> : items.length === 0 ? <div className="p-6 text-center text-sm text-muted-foreground">Nenhuma conversa encontrada.</div> : items.map((item) => { const active = item.id === selectedId; return <button type="button" key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full border-b border-border/70 p-4 text-left transition ${active ? "bg-cyan-400/[0.08]" : "hover:bg-background/70"}`}><div className="flex gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${active ? "bg-cyan-300 text-slate-950" : "bg-muted text-muted-foreground"}`}>{initials(item.user.name, item.user.email)}</span><span className="min-w-0 flex-1"><span className="flex items-start justify-between gap-2"><span className="flex min-w-0 flex-wrap items-center gap-2"><strong className="truncate text-sm text-foreground">{item.user.name || item.user.email}</strong><SubscriptionBadge active={item.user.subscriptionActive} /></span><time className="shrink-0 text-[11px] text-muted-foreground">{formatTime(item.updatedAt)}</time></span><span className="mt-1 block truncate text-xs font-medium text-muted-foreground">{item.title || "Conversa sem título"}</span><span className="mt-1 block truncate text-xs text-muted-foreground/70">{lastMessage(item)}</span></span></div></button> })}</div>
           {pagination && pagination.totalPages > 1 ? <div className="flex items-center justify-between border-t border-border p-3 text-xs"><button type="button" disabled={pagination.page <= 1} onClick={() => void load(pagination.page - 1)} className="rounded-md border border-border px-2.5 py-1.5 disabled:opacity-40">Anterior</button><span className="text-muted-foreground">Página {pagination.page} de {pagination.totalPages}</span><button type="button" disabled={pagination.page >= pagination.totalPages} onClick={() => void load(pagination.page + 1)} className="rounded-md border border-border px-2.5 py-1.5 disabled:opacity-40">Próxima</button></div> : null}
        </aside>
